...
Checkpoint commits on the fork are not PR preparation events. They are local preservation and audit work. Do not create an upstream branch, draft PR, GitHub PR page, issue, or maintainer-facing summary merely because a checkpoint looks reviewable.
...
- remove OpenGGF-specific Markdown documentation, CSV audit packages, roadmap files, review packages, and any OpenGGF-branded process notes from the PR branch before opening the PR; this includes local files such as `OPENGGF_*.md` and `OPENGGF_*.csv`, plus any renamed Markdown/process artifact that still explains OpenGGF's internal roadmap
...
- after the external OpenGGF release-readiness trigger, rename existing labels only after the maintainer style has been established by a small accepted PR or explicit maintainer feedback
...
10. Wait for the external OpenGGF release-readiness trigger before any upstream issue, draft PR, style-confirmation PR, or annotation PR.
11. After that trigger, create clean upstream-facing branches that exclude OpenGGF-specific Markdown/CSV audit files and process documentation before any PR is opened.
